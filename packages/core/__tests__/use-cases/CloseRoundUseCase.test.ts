...
describe('CloseRoundUseCase', () => {
  it('closes an open round', async () => {
    ...
    const useCase = new CloseRoundUseCase(repo)
  ...
  it('rejects if round is not open', async () => {
    ...
    if (!result.ok) expect(result.error.message).toContain('Only open rounds')
  })
